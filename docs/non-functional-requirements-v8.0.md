# Non-Functional Requirements v8.0
...
- master-data service splits should keep `apps.master_data.services` as a
  ...
  Unit service modules, to Employee administration/service-boundary modules,
  and to extracted reference/master service modules.
...
- Azure development runs the hardened production configuration profile against
  dedicated dev resources so unsafe local-only settings are caught before test
  or production promotion.
- Azure App Service package detection must install from `requirements.txt`, and
  the App Service startup command must run `bash scripts/azure-startup.sh`.
- The dev App Service uses Google SSO through App Service Authentication and
  Key Vault-backed secrets.
- KAN-121 verified that migrations and idempotent reference/dev seed commands
  run successfully against Azure Database for PostgreSQL Flexible Server.
- Historical Country-to-Office migrations must remain PostgreSQL-compatible,
  including normalization of preserved index names and sequence values before
  recreating the standalone Country table.

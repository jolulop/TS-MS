# Integration API Specification v8.0

## 1. Purpose

Describe the currently implemented JSON API surface under `/api/v1`.

No JSON API versioning change was introduced in the v8.0 documentation promotion. The main implemented deltas in the current surface are:
...
- Azure App Service Google SSO uses the `trusted-header` adapter for browser
  session bootstrap before all internal API authorization checks
...
- `TSMS_DB_SSLMODE=require` and a bounded `TSMS_DB_CONN_MAX_AGE` are used for
  Azure PostgreSQL parity.
...
- Azure App Service deployments use `requirements.txt` for Oryx dependency
  detection and `bash scripts/azure-startup.sh` as the startup command.
...
- documented `DELETE` endpoints in this section are implemented JSON endpoints
- successful deletes return HTTP 200 with `deleted`, `entity`, and `id`
- blocked deletes return a structured error response with a stable
  `*_DELETE_BLOCKED` code and leave the protected record in place
- guarded deletes do not cascade-delete business data; limited setup or
  relationship cleanup occurs only where the service-layer delete rule
  explicitly owns that cleanup
- blocked delete attempts are audited where the service-layer rule identifies
  them as guarded-delete denials
...
- Cross-Office Staffing create, update, and delete workflow
- personal timesheet deletion from the HTML editor

# UI Screen Specification v6.0

## 1. Purpose

Describe the approved v6.0 server-rendered UI screens and behaviors for the
repository baseline.
...
- Detail:
  ...
  - show a read-only active Cross-Office Staffing grid for the employee when
    relevant
  - guarded delete action
...
- employee detail shows active scoped project assignments in a grid with columns
  `Name`, `BU`, `From`, and `To`
- employee detail may show active cross-office staffing with origin Office,
  target project, and staffing dates when the employee participates in
  cross-office project work

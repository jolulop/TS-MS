...
Summarize the approved v8.0 logical data model and point to the detailed
...
- v8.0 includes approved logical support for dedicated cross-office staffing on
  top of the prior admin API parity, reporting, and General Charge Code
  routing-governance baseline.
- v8.0 also records the PostgreSQL migration compatibility requirement for the
  historical Country-to-Office rename: preserved database artifacts on the
  renamed `office` table must not block recreating the standalone `country`
  table, and the Office sequence must remain aligned after explicit-id seed
  history.
...
and the v8.0 markdown set as the approved source-of-truth reference.

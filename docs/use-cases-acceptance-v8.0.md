# Use Cases And Acceptance v8.0

## 0. Azure Development Instance

### Use Case

The team operates a disposable-but-working Azure development instance for
Milestone 1 validation without using local SQLite or local development auth.

### Acceptance

- `app-tsms-dev` is deployed in the isolated `rg-tsms-dev` resource group
- Azure dev runs Django with `TSMS_ENVIRONMENT=production`
- Google SSO through App Service Authentication protects browser access
- Django initializes the internal TS session from the trusted ingress email
  claim
- Azure dev uses Azure Database for PostgreSQL Flexible Server, not SQLite
- `python manage.py migrate --noinput` succeeds against the dev PostgreSQL
  database
- `python manage.py migrate --check` succeeds after migration
- `seed_reference_data` and `seed_dev_data` are idempotent against the dev
  PostgreSQL database
- an authenticated user with an internal employee record can open the deployed
  TSMS app through the Azure URL
